...
import { FaLeaf, FaTimes, FaHospital } from 'react-icons/fa';
...
function Health() {
  ...
  const filteredItems = health.filter((item) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[60vh] bg-emerald-900 overflow-hidden">
        {/* Logo */}
        <div
          className={`absolute top-10 left-10 z-[55] transition-opacity duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}
        >
        ...
        {/* Navigation */}
        <div
          className={`absolute top-9 left-1/2 -translate-x-1/2 hidden lg:block z-[65] transition-opacity duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}
        >
        ...
        <div className="relative z-20 h-full flex items-center justify-center">
          <div className="text-center text-white">
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-6">Fasilitas Kesehatan</h1>
            <p className="text-xl md:text-2xl text-white/90">
              Layanan Kesehatan Terpercaya di Kota Bogor
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
            />
          </div>
        </div>
      </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredItems.map((item, index) => (
            <motion.div
              ...
              onClick={() => {
                window.location.href = `/health/${item.slug}`;
              }}
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="text-emerald-400 text-sm flex items-center gap-2">
                    <FaHospital className="w-4 h-4" /> Fasilitas Kesehatan
                  </span>
                </div>
                ...
                <h3 className="text-2xl font-normal mb-4 group-hover:text-emerald-800 transition-colors">
                  {item.title}
                </h3>
                <p className="text-gray-600 leading-relaxed mb-6 flex-grow line-clamp-2">
