...
// Variants animasi saat transit ke/keluar halaman
...
function Destinations() {
  ...
  // State untuk menu overlay dan pencarian
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // MenuItems untuk MenuOverlay
  const menuItems = [
  ...
  // Variants untuk container dan item (efek fade + slide)
  ...
  return (
    ...
    >
      ...
      {/* Hero Section (sama seperti di Kuliner) */}
      <div className="relative h-[60vh] bg-emerald-900 overflow-hidden">
        ...
        {/* Overlay kosong jika perlu */}
        <div className="absolute inset-0 z-10" />

        {/* Title & subtitle */}
        <div className="relative z-20 h-full flex items-center justify-center">
          <div className="text-center text-white">
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-6">Destinations</h1>
            <p className="text-xl md:text-2xl text-white/90">Temukan Keindahan Kota Bogor</p>
          </div>
        </div>
      </div>

      {/* Search Section (sama seperti di Kuliner) */}
      <motion.div
      ...
      {/* Content Grid (mirip Kuliner), tapi langsung link ke halaman detail */}
      <motion.div
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredItems.map((item, index) => (
            ...
            >
              {/* Seluruh card di-_wrap_ oleh Link untuk menuju detail berdasarkan slug */}
              <Link
              ...
              >
                ...
                <div className="flex flex-col h-full">
                  {/* Nomor + label (pakai ikon FaUtensils biar mirip) */}
      ...
      {/* Tidak ada lagi modal detail karena kita langsung link ke halaman detail */}

      {/* Chatbot */}
      <ChatBot isMenuOpen={isMenuOpen} />

      {/* Footer (copy dari Kuliner) */}
      <Footer />
    </motion.div>
  );
}
